...
In this post, we install `RCall` programmatically from within the document itself, which will trigger the build process after installation, so there is no need to manually call `Pkg.build("RCall")` in this case.
